        ...
        while ((chosenDC = menu.premenu(allDCs)) != 9) {
            if (chosenDC == (allDCs.getDCamount() + 1)) {
                ...
                while ((userChoice = menu.mainAllMenu()) != 9) {
                    switch (userChoice) {
                        case 1:
                            menu.showAllDCsEnergyCosts(CC.getAllDcsEnergyCosts());
                            break;
                        case 2:
                            menu.showDCMaxEnergyCosts(CC.getDCMaxEnergyCost());
                            break;
                        default:
                            break;
                    }
                ...
                Response msg = CC.changeEnergyCost(menu.newEnergyCost());
                System.out.println("Response: " + msg.getMessage() + " status: " + msg.isStatus());
                input.nextLine();
                ...
                while ((userChoice = menu.mainSingleMenu(getDCfromIndex(chosenDC))) != 9) {
                    switch (userChoice) {
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            menu.showFullEnergyReport(CC.getFullEnergyReport(chosenDC));
                            break;
                        default:
                            break;
                    }
