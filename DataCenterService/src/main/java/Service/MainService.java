...
public class MainService {
    ...
    @GET
    @Path("get/fullenergyreport/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public EnergyReport getFullEnergyReport(@PathParam("id") int id) {
        return db.getDCFullEnergyReport(id);

    }

    @GET
    @Path("get/maxenergycost")
    @Produces(MediaType.APPLICATION_JSON)
    public Data getDCMaxEnergyCost() {
        return db.getDCMaxEnergyCost();
    }

    @GET
    @Path("get/allenergycost")
    @Produces(MediaType.APPLICATION_JSON)
    public EnergyReport getAllDCsEnergyCosts() {
        return db.getAllDcsEnergyCosts();

    }

    @POST
    @Path("add/newenergycost")
    @Produces(MediaType.APPLICATION_JSON)
    public Response addNewEnergyCostPost(Data data) {
        Response res = new Response("New energy price added", Boolean.FALSE);
        if (db.addNewEnergyPrice(data)) {
